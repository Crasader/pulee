...
#define TableViewItemImageHeightPercentage 75
#define TableViewItemImageTitleSeparationPercentage 0
...
bool Categories::init()
{
    ...
    auto labelCategoriesTitle = Label::createWithTTF(LanguageManager::getLocalizedText("Categories", "title"), MainBoldFont, 70);
    labelCategoriesTitle->setAlignment(TextHAlignment::CENTER);
    labelCategoriesTitle->setAnchorPoint(Point::ANCHOR_MIDDLE_BOTTOM);
    labelCategoriesTitle->setTextColor(IkasBlue);
...
PagingTableViewCell* Categories::tableCellAtIndex(PagingTableView *table, ssize_t idx)
{
    ...
    labelSize.height = labelSize.height * (1 - (float)TableViewItemImageHeightPercentage/100 - (float)TableViewItemImageTitleSeparationPercentage/100);
    auto label = Label::createWithTTF(string->getCString(), MainBoldFont, 64);
    
    label->setContentSize(labelSize);
    label->setWidth(labelSize.width);
    label->setAnchorPoint(Point::ANCHOR_MIDDLE);
    label->setPosition(Vec2(itemSize.width / 2, labelSize.height / 2));
